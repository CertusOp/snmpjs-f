...
var DEFAUT_BIND_PORT = 161;
var DEFAULT_FAMILY = 'udp4'; // this is the `type` param of dgram.createSocket
...
Agent.prototype.listen = function(cb) {
  this.bind({family: DEFAULT_FAMILY, port: DEFAUT_BIND_PORT}, cb);
  // TODO listen on all udp6 by default too?
}
...
Agent.prototype._varbind_set_single =
	...
	if (typeof (vb) === 'undefined' && req.pdu.op == PDU.GetNextRequest) {
		...
		if (req.pdu.op != PDU.SetRequest && vb != PDU.genErr) {
			this._log.warn({ snmpmsg: req },
			    'provider attempted to set prohibited  error code %s for varbind %d',
			    vb, idx);
			vb = PDU.genErr;
		}
		rsp.pdu.varbinds[idx] = req.pdu.varbinds[idx].clone();
...
Agent.prototype._do_getset = async function _do_getset(req, rsp) {
	var self = this;
	var nvb = req.pdu.varbinds.length;
	var ndone = 0;

	await asyncForEach(req.pdu.varbinds, async function (vb, i) {
		var node = self._mib.lookup(vb.oid);
		var prq = new ProviderRequest(req.pdu.op, vb.oid, node);
		var decor = node.decor('_agent');
		var handler;

		prq.community = req.community;
		...
		await asyncForEach(handler, async function (h) {
			await h(prq);
		});
	});
};
...
Agent.prototype._do_getnext_one =
    async function (req, rsp, i, oid, cookie, first) {
	var self = this;
	...
	prq._done = async function _getnext_done(rsvb) {
		if (rsvb !== undefined) {
			self._varbind_set_single(req, rsp, i, rsvb);
			if (++cookie.ndone == nvb)
				self._transmit_response(req._conn_recv, rsp);
		...
		await self._do_getnext_one(req, rsp, i, prq.node.oid, cookie, false);
	};
...
Agent.prototype._do_getnext = async function _do_getnext(req, rsp) {
	var self = this;
	var cookie = { ndone: 0 };

	await asyncForEach(req.pdu.varbinds, async function (vb, i) {
		await self._do_getnext_one(req, rsp, i, vb.oid, cookie, true);
	});
};
...
Agent.prototype._process_req = async function _process_req(req) {
	var rsp;
	...
	case PDU.GetRequest:
	case PDU.SetRequest:
		await this._do_getset(req, rsp);
		break;
	case PDU.GetNextRequest:
		await this._do_getnext(req, rsp);
		break;
	case PDU.GetBulkRequest:
		await this._do_getbulk(req, rsp);
		break;
	case PDU.Response:
	case PDU.Trap:
	...
	case PDU.Report:
	default:
		await Listener.prototype._process_msg.call(this, req);
		break;
	}
};

Agent.prototype._process_msg = async function _process_msg(msg) {
	await this._process_req(msg);
};
