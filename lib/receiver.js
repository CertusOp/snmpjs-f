...
Receiver.prototype._recv = async function _recv(raw, src, conn) {
	var msg;
...
Receiver.prototype.createSocket = function createSocket(family) {
	...
	conn.on('message', async function _recv_binder(msg, rinfo) {
		var raw = {
			buf: msg,
			len: rinfo.size
		};
		...
		await self._recv(raw, src, conn);
		await self._resp(raw, src, conn);
	});
